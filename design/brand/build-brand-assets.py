...
def email_logo(lockup: Image.Image) -> None:
    """The header logo for email, emitted as a TypeScript module.

    Mail clients block remote images from a sender the reader has not trusted,
    so the logo travels inside the message as an inline part instead of being
    fetched from pioassets.com. That makes it a runtime dependency of the API,
    and a .png sitting in src/ is exactly the kind of thing a build or a
    container copy drops silently - as base64 in a .ts file it cannot go missing.

    The white ground is baked in rather than left transparent: a client that
    force-darkens the mail would otherwise put the navy lettering on a dark
    background and erase half the wordmark.
    """
    width, height = 392, 102  # 2x the 196x51 it is displayed at
    art = lockup.resize((width, round(lockup.height * width / lockup.width)), Image.LANCZOS)
    plate = Image.new("RGBA", (width, height), (255, 255, 255, 255))
    plate.paste(art, (0, (height - art.height) // 2), art)
    buf = io.BytesIO()
    plate.convert("RGB").quantize(colors=192, method=Image.FASTOCTREE).save(
        buf, format="PNG", optimize=True
    )
    data = base64.b64encode(buf.getvalue()).decode("ascii")

    out = ROOT / "apps" / "api" / "src" / "notifications" / "brand-logo.generated.ts"
    ...
    out.write_text(
        ...
        f"/** Displayed at {width // 2}x{height // 2}; the art is 2x for retina. */\n"
        f"export const BRAND_LOGO_WIDTH = {width // 2};\n"
        f"export const BRAND_LOGO_HEIGHT = {height // 2};\n\n"
        "export const BRAND_LOGO_BASE64 =\n" + body + "\n",
        encoding="utf-8",
    )
    print(f"  {out.relative_to(ROOT)}  {width}x{height}  {len(data) // 1024} KB base64")
