...
**Getting there:** [Implementation Plan v2.1](./docs/IMPLEMENTATION-PLAN-v2.1.md) sequences the blueprint into shippable releases (v2.1–v2.6) mapped to the real codebase. v2.1 (foundation & status model), v2.2 (core ITAM & RBAC), v2.3 (License Management, the flagship seat-limit-enforcement module), v2.4 (Procurement & Warehouse Inventory) and v2.5 ([Discovery, Hardware Detail & Work Orders](./docs/IMPLEMENTATION-PLAN-v2.5-HARDWARE.md)) are live; v2.6 ([Analytics, Integrations & the Platform Plane](./docs/IMPLEMENTATION-PLAN-v2.6-ANALYTICS.md)) completed the mapped plan; and v2.7 ([Trust Hardening & License Completion](./docs/IMPLEMENTATION-PLAN-v2.7-HARDENING.md), the first post-roadmap release) are live — RLS tenant isolation is now enforced at the database. v2.8 ([Operational Resilience & Observability](./docs/IMPLEMENTATION-PLAN-v2.8-RESILIENCE.md)) is live — off-site backups, a self-running restore drill, a deploy preflight, opt-in tracing and per-tenant rate limits. v2.9 ([Procurement & Inventory Completion](./docs/IMPLEMENTATION-PLAN-v2.9-PROCUREMENT-COMPLETION.md), [QA scorecard](./docs/QA-SCORECARD-v2.9.md)) closes v2.4's deliberate deferrals: receiving an asset now creates the asset, budgets are hard limits rather than warnings, competitive quoting has an award that is the only route to an order, and consumables track lots with FIFO-by-expiry issue. v2.10 ([Scale](./docs/IMPLEMENTATION-PLAN-v2.10-SCALE.md), [QA scorecard](./docs/QA-SCORECARD-v2.10.md)) proved the product at enterprise volume and fixed what the evidence showed: at 100k assets and 2M audit rows, every endpoint now meets its latency target, the nightly sweep dropped from 45s to 11s, and a 100k-row export holds 1.4 MB instead of 16.4 MB.
